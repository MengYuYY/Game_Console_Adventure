...
cc.Class({
	...
	UpBegin(){
		if (!this.isUp && this.physicsCheck.isTouchLadder) {
			...
			//停止之前攀爬音效
			this.audioPlay.stopEffects(1);
			//播放攀爬音效
			this.audioPlay.playerClimb();
		}
	},
	...
	LeftBegin(){
		if (!this.isLeft && !this.isDuck){
			...
			if (this.physicsCheck.isGround){
				...
				//停止之前行走音效
				this.audioPlay.stopEffects(0);
				//播放移动音效
				this.audioPlay.playerWalk();
			}
		}
	},
